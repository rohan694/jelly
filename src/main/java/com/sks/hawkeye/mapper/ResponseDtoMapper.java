...
public class ResponseDtoMapper implements RowMapper<DataResponse> {
	...
	public DataResponse mapRow(ResultSet rs, int rowNum) throws SQLException {
		...
		try {
			...
			dto.setImpactPosition(new ReleasePosition(rs.getDouble("impact_position_x"),rs.getDouble("impact_position_y"),rs.getDouble("impact_position_z")));
			dto.setCreasePosition(new ReleasePosition(rs.getDouble("crease_position_x"),rs.getDouble("crease_position_y"),rs.getDouble("crease_position_z")));
